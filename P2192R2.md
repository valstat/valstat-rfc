...
This is a proposal about logical, feasible, lightweight and effective handling of information returned from functions, based on an paradigm shift. This proposal is **not** yet another error handling solution, based on some elaborate new type implemented in snazzy C++. 

This paper proposes an paradigm shift. Utility to a solution to a large proportion of deeply rooted issues [[3](#ref3)][[15](#ref15)]. Implemented, this would be a std library solution without a language change required. Materialized in standard C++ as one tiny template. 
...
As of today, in the std lib, there are few (or more than few) non interoperable error handling paradigms, idioms and return types. From ancient to contemporary, but still years old by now. None of them dealing with general function returns in an satisfactory manner. Added through time they have inevitably contributed to a rising technical debt present inside C++ std lib. 

What is "Returns handling"? Returns handling is the evolution of error handling. It is the true picture of real life, API consuming algorithms, required to deal with wider scope of issues, opposed to a simple "was there an error or not?" logic. Hint: there was something but it is not an error.

There is none of such std lib pervasive, paradigm and mechanism. Dotted on the wast C++ landscape are some "home grown" solutions to the general returns handling in large C++ code bases. Think HTTP codes handling, as an example. Lack of a common and ubiquitous non trivial returns handling solutions is raising the level of complexity for all levels of application architectures developed with standard C++ and std lib. 

For reasons of developing modern application architectures, requiring  general returns handling, developed in general for (multilingual) distributed systems , author has formalised the "**metastate**" paradigm. 
...
Key reason for appearance of C++ dialects, are to be found in strict run-time requirements. That means developing using the core language but [without the std lib](https://github.com/electronicarts/EASTL). Without std lib error handling idioms and related types.  

One motivation of this paper is to try and offer an "over arching", but simple enough, returns handling paradigm applicable across standard C++. And possibly across fast growing C++ dialects, fragmenting away the industry and markets. 

**Minimal list**

Honourable readership is aware of all the core reasons behind these requirements. 

1. can not use try / throw / catch[[6](#ref6)][[15](#ref15)] 
2. can not use \<system_error\>[[14](#ref14)]
3. do not use `iostreams`

 Author will be so bold not to delve into further detail in order to keep this paper simple and focused. Gaming, embedded systems, high performance computing, are just the tip of the iceberg to which C++ ship might be heading. Perhaps pathetic but true.

### 2.3. Interoperability

Each solution to strict run-time requirements is adding yet another nail in the coffin of interoperability. In danger of sounding like offering an panacea,  author will also draw the attention to the malleability of the metastate paradigm to be implemented with wide variety of languages used in developing components of an modern distributed system. 
...
1. no "error code" as return value
   ...
   2. Increasing the learning curve for every function in every API
   3. How to decode the error code?
2. no "return arguments" 
   1. language specific, mutable argument solutions are definitely not interoperable.  
...
Data centers energy consumption has become imperative. Most of the server side software is written in C/C++ . Pressure is on, to design and develop using standard C++ also with energy consumption as an primary requirement. 

This one is not "simple". Somewhat paradoxically this category of requirements requires less and less code. Smaller code and smaller executables means less energy.
